...
import { Loader2, Minus, Plus, Info, Utensils, Scan, CreditCard } from 'lucide-react';
...
export default function OwnerOrderPage() {
  ...
  if (user && reservation.tentOwnerId !== user.uid) {
    return <p>Você não tem permissão para gerenciar este pedido.</p>
  }
  ...
  const handleAddItemsToReservation = async () => {
    ...
    const newItems: ReservationItem[] = Object.values(cart).map(({ item, quantity }) => ({
      ...
      price: item.price,
      quantity: quantity,
      status: 'confirmed', // Items added by owner are auto-confirmed
    }));

    const updateData = {
      items: arrayUnion(...newItems),
      total: reservation.total + newItemsTotal
    };

    updateDoc(reservationRef, updateData)
      .then(() => {
        toast({
          title: "Itens Adicionados!",
          description: `${Object.keys(cart).length} novo(s) item(ns) foram adicionados ao pedido.`,
        });
        setCart({});
      })
      .catch((error) => {
  ...
  return (
    <div className="w-full max-w-5xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Gerenciar Pedido</h1>
        <div className="text-muted-foreground flex items-center gap-4">
          <p>Cliente: <span className="font-semibold">{reservation.userName}</span></p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-8">
        <div className="lg:col-span-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Adicione ou remova itens do pedido do cliente.</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="lg:col-span-1 mt-8 lg:mt-0">
          <Card className="sticky top-24">
            <CardHeader>
              <CardTitle>Itens Atuais e Novos</CardTitle>
              <CardDescription>Resumo do pedido.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <h4 className="font-semibold text-sm mb-2">Itens no Pedido</h4>
                {reservation.items.length > 0 ? (
                  <ul className="space-y-2 text-sm text-muted-foreground">
                    {reservation.items.filter(i => i.status === 'confirmed').map((item, index) => (
                  ...
                  <p className="text-center text-muted-foreground text-sm">Nenhum item confirmado.</p>
                )}
              </div>
              <div className="border-t pt-4">
                 <h4 className="font-semibold text-sm mb-2">Novos Itens a Adicionar</h4>
                {isCartEmpty ? (
                    <p className="text-center text-muted-foreground text-sm">Nenhum item novo.</p>
                ) : (
                    <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <CardFooter className="flex-col items-stretch gap-4">
                <div className="flex justify-between items-baseline border-t pt-4">
                    ...
                    <p className="text-2xl font-bold">R$ {(reservation.total + newItemsTotal).toFixed(2)}</p>
                </div>
